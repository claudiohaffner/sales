...
A Tax is described by a type, a rate (%) and a generic TaxLevyRule that simply defines whether a tax is applicable on a product or not. 
...
A TaxPolicy holds all the defined taxes and the general rules of rounding, in order to calculate the total tax price.
...
This application provides two examples of receipts: the ClientReceipt (which shows the simple output required by the exercise) and the DetailedReceipt
(which shows all the details about the taxes applied on every single ProductOrder). In this simple implementation all the types of Receipt print 
on System.out, but it's easy to implement other wrappers that write a receipt on file or on other output streams.
...
the requirements ("rounding rules apply to the shelf price") we dedided to put ON_PRODUCT rounding by default.
...
- testQuantity: testing a shopping basket with quantity >1
